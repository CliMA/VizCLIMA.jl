...
# data folders
data_folder = "/central/scratch/bischtob/gcm_hs/netcdf/"
combined_file = "/central/scratch/bischtob/gcm_hs/netcdf/combined.nc"
analysis_folder = "/central/scratch/bischtob/gcm_hs/analysis/"
...
p2 = contourf(lat, z, data_time_zonal_mean["v"], ylim=ylim, color=cg, levels=LinRange(-4, 4, nlev), linewidth=0)
...
fig = plot(p1, p2, p3, p4, p5, p6, layout=(3, 2), size=(2500, 1000), title=["u (m/s)" "v (m/s)" "cov(u,v) (m^2/s^2)" "cov(v,T) (Km /s^2)" "T (K)" "var(T) (K)"])
...
p4 = plot(data_time_zonal_mean["T"][:, [5, 10, end-4]], z, xlabel="Total energy (J)", ylabel="z (km)")
...
p4 = plot(data_time_zonal_mean["T"][[1, 5, 15], :]', lat, xlabel="Total energy (J)", ylabel="latitude", label=false)
fig = plot(p1, p2, p3, p4, layout=(2, 2), size=(1000, 800), title=["Zonal & time mean zonal velocity (m/s)" "Zonal & time mean meridional velocity (m/s)" "Zonal & time mean vertical velocity (m/s)" "Total energy (J)"])
...
fig = plot(time[start:end], ke, size=(1250, 333), xlabel="time", ylabel="squared norm of velocity vector")
